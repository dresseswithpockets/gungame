...
func update_properties():
    ...
    # Omni settings
    var radius: float = properties.get("radius", default_radius)
    radius *= Constants.TbToGodot
    var attenuation: float = properties.get("attenuation", 1.0)
    ...
    light_node.omni_range = radius
    light_node.omni_attenuation = attenuation
